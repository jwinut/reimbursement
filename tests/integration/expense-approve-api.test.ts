...
function createRequest(id: string, csrfToken = 'valid-token'): NextRequest {
  const headers = new Headers()
  headers.set('X-CSRF-Token', csrfToken)
  headers.set('Cookie', `csrf-token=${csrfToken}`)
  headers.set('Content-Type', 'application/json')

  return new NextRequest(`http://localhost:3000/api/expenses/${id}/approve`, {
    method: 'POST',
    headers,
  })
}
...
describe('POST /api/expenses/[id]/approve', () => {
  ...
  it('should return 400 for expense not in PENDING status', async () => {
    ...
    expect(data.message).toContain('Cannot approve expense')
  })

  it('should successfully approve PENDING expense', async () => {
    const mockExpense = {
      id: 'expense-123',
    ...
    expect(data.status).toBe(ExpenseStatus.APPROVED)
    expect(data.approverId).toBe('manager-123')
  })

  it('should set approverId and approvalDate on approval', async () => {
    const mockExpense = {
      id: 'expense-123',
      status: ExpenseStatus.PENDING,
      userId: 'user-123',
